...
function Parser(options) {
    ...
    this.max_buffer_size = options.max_buffer_size || 1024;
...
Parser.prototype._transform = function(data, encoding, callback) {
    ...
    if (!lines[0]) {
        if ((this.held_data ? this.held_data.length : 0 ) + data.length > this.max_buffer_size) {
    ...
    if (line_start < data_pos) {
        if ((data.length - line_start) > this.max_buffer_size) {
            // Buffering this data would exeed our max buffer size
            this.emit('error', 'Message buffer too large');
            return callback();
        }
