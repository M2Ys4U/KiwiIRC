define('ui/messagelist/message_view', ['lib/backbone', 'ui/application', 'ui/mediamessage', 'helpers/styletext', 'helpers/translator', 'helpers/hsl2rgb', 'helpers/formatircmsg', 'helpers/emoticonfromtext', 'helpers/settings'], function (Backbone, Application, mediaMessage, styleText, translator, hsl2rgb, formatIRCMsg, emoticonFromText, settings) {
    return Backbone.View.extend({
        ...
        render: function() {
            ...
            this.display.nick = styleText('message_nick', {nick: this.display.nick, prefix: this.display.nick_prefix || ''});
        ...
        getNickStyles: (function () {
            ...
            return function (nick) {
                ...
                nick_lightness = (_.find(Application.instance().themes, function (theme) {
                    return theme.name.toLowerCase() === settings.get('theme').toLowerCase();
                }) || {}).nick_lightness;
                ...
                return {
                    color: '#' + ('000000' + (rgb[2] | (rgb[1] << 8) | (rgb[0] << 16)).toString(16)).substr(-6), //jshint ignore:line
        ...
        generateMessageDisplayObj: function(msg) {
            ...
            if (nick && msg.nick.localeCompare(nick) !== 0) {
                ...
                regexpStr = _.chain((settings.get('custom_highlights') || '').split(/[\s,]+/))
                    .compact()
                    .concat(nick)
                    .map(_.escapeRegExp)
                    .join('|')
            ...
            message_words = _.map(message_words, function(word) {
                ...
                if (settings.get('show_emoticons')) {
            ...
            if (settings.get('use_24_hour_timestamps')) {
                ...
                msg.time_string = translator.translateText(am_pm_locale_key, hour + ":" + msg.time.getMinutes().toString().lpad(2, "0") + ":" + msg.time.getSeconds().toString().lpad(2, "0"));
            }
